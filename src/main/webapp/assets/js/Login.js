		...
		const signup = () => {
			...
			let type = document.getElementById("user").checked === true ? role1 : role2;
			//document.getElementById("user").checked==true?type=role1:type=role2
		
			let address = document.getElementById("address").value;
			const url =
				"http://localhost:8080/charitytrust-webapp/SignUpServlet?name=" +
				Name +
				"&email=" +
				email +
				"&pass=" +
			...
			password === confirmpassword
				...
				: swal(
					"sorry!",
					"new password and confirm password or not matchced",
					"error"
				);
		};
		async function fetchSignup(url) {
			try {
				...
				if (response.ok) {
					...
					responseData.trim() === "Successfully Added User"
						? success()
						: swal("Error!", responseData, "error");
				}
				else {
						handleServerError(response.status,responseData);
					}
			} catch (error) {
		...
		passwordCheckbox.addEventListener("change", function () {
			this.checked
				? ((passwordInput.type = "text"), (confirmpasswords.type = "text"))
				: ((passwordInput.type = "password"), (confirmpasswords.type = "password"));
		});
		...
		const login = () => {
			...
			const url =
				"http://localhost:8080/charitytrust-webapp/CheckEmailServlet?email=" +
				userloginmail +
				"&pass=" +
				userloginpassword;
			fetchLogin(url);
		...
		async function fetchLogin(url) {
			try {
				...
				if (response.ok) {
					...
					responseData.trim() == "Successfully LoggedIn"
						? pageRedirect()
						: swal("Error!", responseData, "error");
				}
		...
		function delayedAction() {
			const usermail = document.getElementById("logemail").value;
			const redirectUrl = usermail === "freshtrust@gmail.com" ? "adminEvent.jsp" : "Profile.jsp";
